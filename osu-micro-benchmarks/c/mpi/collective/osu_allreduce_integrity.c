#define BENCHMARK "OSU MPI%s Allreduce Latency Test w/ integrity check"
/*
...
int main(int argc, char *argv[])
{
    ...
    for(size=options.min_message_size; size*sizeof(int) <= options.max_message_size; size *= 2) {
        ...
        for(i=0; i < options.iterations + options.skip ; i++) {
            for(j = 0; j < size; j++)
                sendbuf[j] = (i * (size + j)) * (rank + 1);

            MPI_CHECK(MPI_Barrier(MPI_COMM_WORLD));

            t_start = MPI_Wtime();
            MPI_CHECK(MPI_Allreduce(sendbuf, recvbuf, size,
            ...
            for(j = 0; j< size; j++) {
                ...
                int expected = (i * (size + j)) * (numprocs * (numprocs + 1) / 2);
                int v = (int) recvbuf[j];

                if(expected != v) {
                    printf("Error: size = %zu, i = %d k = %d. Expected %d, got %d. sendbuf[%d] is %d\n",
                        size * sizeof(int), i, j, expected, v, j, sendbuf[j]);
