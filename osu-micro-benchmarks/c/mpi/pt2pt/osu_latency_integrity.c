...
main (int argc, char *argv[])
{
    ...
    if(numprocs != 2) {
        if(myid == 0) {
            fprintf(stderr, "This test requires exactly two processes, got %d\n", numprocs);
        }
    ...
    for(size = options.min_message_size; size <= options.max_message_size; size = (size ? size * 2 : 1)) {
		...
		set_buffer_pt2pt(s_buf, myid, options.accel, size%150, size);
        set_buffer_pt2pt(r_buf, myid, options.accel, (size%150)+1, size);
        ...
        if(myid == 0) {
            for(i = 0; i < options.iterations + options.skip; i++) {
				...
				assert(memcmp(s_buf,r_buf, size) == 0);
            }
