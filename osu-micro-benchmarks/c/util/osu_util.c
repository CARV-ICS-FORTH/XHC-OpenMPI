...
static long long decode_size_mod(char *str) {
    ...
    char *numeric = strdup(str);

    if(mult != 1)
        numeric[len - 1] = '\0';

    long long val = atoll(numeric) * mult;
    free(numeric);
...
static int set_message_size (char *val_str)
{
    ...
    if (!count) {
        retval = set_max_message_size(decode_size_mod(val_str));
    } else if (count == 1) {
        val1 = strtok(val_str, ":");
        val2 = strtok(NULL, ":");

        if (val1 && val2) {
            retval = set_min_message_size(decode_size_mod(val1));
            retval = set_max_message_size(decode_size_mod(val2));
        } else if (val1) {
            if (val_str[0] == ':') {
                retval = set_max_message_size(decode_size_mod(val1));
            } else {
                retval = set_min_message_size(decode_size_mod(val1));
            }
        }
