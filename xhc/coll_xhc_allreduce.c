...
#define LEADER_RD_POLICY (OMPI_XHC_ALLREDUCE_LEADER_REDUCE_POLICY)
#if LEADER_RD_POLICY < 0 || LEADER_RD_POLICY > 3
	#error Invalid leader reduce policy value
#endif
...
static void xhc_allreduce_calc_chunk_sizes(xhc_comm_t *comms,
		int comm_count, int allreduce_count, size_t dtype_size) {
	
	for(int i = 0; i < comm_count; i++) {
		xhc_comm_t *xc = &comms[i];
		
		#if LEADER_RD_POLICY == 0
			xc->reduce_worker_count = xc->size - 1;
		#elif LEADER_RD_POLICY == 1 || LEADER_RD_POLICY == 2
			xc->reduce_worker_count = (i == comm_count - 1 ? xc->size : xc->size - 1);
		#elif LEADER_RD_POLICY == 3
			xc->reduce_worker_count = xc->size;
		#endif
		
		size_t bytes_per_member;
		
		/* Members split up the work according to the provided chunk
		 * size MCA parameter. If "uniform chunks" are enabled, when
		 * the requested chunk size is too large (comparatively to the
		 * reduce count) for all ranks to do work, a lower chunk size
		 * will be selected for the specific operation. */
		
		if(mca_coll_xhc_component.uniform_chunks) {
			bytes_per_member = opal_min(xc->chunk_size,
				(allreduce_count * dtype_size)/xc->reduce_worker_count);
			
			bytes_per_member = opal_max(bytes_per_member, dtype_size);
			
			bytes_per_member = opal_max(bytes_per_member,
				mca_coll_xhc_component.uniform_chunks_min);
		} else
			bytes_per_member = xc->chunk_size;
		
		xc->coll_chunk_elements = bytes_per_member / dtype_size;
	}
}

static void xhc_allreduce_init_local(xhc_comm_t *comms,
		int comm_count, int allreduce_count, xf_sig_t seq) {
	...
	for(int i = 0; i < comm_count; i++) {
		...
		int initial_reduce_index = xc->coll_chunk_elements * (xc->member_id - 1);
		
		bool leader_assisting;
		
		#if LEADER_RD_POLICY == 0
			leader_assisting = false;
		#elif LEADER_RD_POLICY == 1
			leader_assisting = (i == comm_count - 1);
		#elif LEADER_RD_POLICY >= 2
			leader_assisting = true;
		#endif
		
		if(leader_assisting)
			initial_reduce_index += xc->coll_chunk_elements;
		
		if(!leader_assisting && xc->is_coll_leader)
			initial_reduce_index = allreduce_count;
...
static void xhc_allreduce_init_member(xhc_comm_t *comms, int comm_count,
		xhc_rank_info_t *rank_info, void *sbuf, void *rbuf, int allreduce_count,
	...
	for(int i = 0; i < comm_count; i++) {
		...
		// Essentially the value of initial_reduce_idx, set in init_local()
		...
		if(!do_cico) {
			...
			xc->my_member_info->sbuf = xhc_get_cico(rank_info, cico_id);
			xc->my_member_info->rbuf = xhc_get_cico(rank_info, ompi_rank);
		}
...
static int xhc_allreduce_attach_member(xhc_comm_t *xc, int member,
		xhc_rank_info_t *rank_info, size_t bytes, bool do_cico, xf_sig_t seq) {
	...
	if(!do_cico) {
		...
		xc->member_info[member].sbuf = xhc_get_registration(
			&rank_info[member_rank], sbuf_vaddr, bytes,
		...
		if(rbuf_vaddr) {
			if(rbuf_vaddr != sbuf_vaddr) {
				xc->member_info[member].rbuf = xhc_get_registration(
					&rank_info[member_rank], rbuf_vaddr, bytes,
		...
		xc->member_info[member].sbuf = xhc_get_cico(rank_info,
			xc->member_ctrl[member].cico_id);
		
		if(CHECK_FLAG(&xc->comm_ctrl->coll_seq, seq, 0)
				&& member == xc->comm_ctrl->leader_id) {
			xc->member_info[member].rbuf = xhc_get_cico(rank_info, xc->manager_rank);
		}
	}
...
static void xhc_allreduce_cico_publish(xhc_comm_t *xc, void *data_src,
		xhc_rank_info_t *rank_info, int ompi_rank, int allreduce_count,
		size_t dtype_size) {
	
	int ready = xc->my_member_ctrl->reduce_ready;
	int elements = opal_min(xc->coll_chunk_elements, allreduce_count - ready);
	
	void *src = (char *) data_src + ready * dtype_size;
	void *dst = (char *) xhc_get_cico(rank_info, ompi_rank) + ready * dtype_size;
...
static int xhc_allreduce_reduce_get_next(xhc_comm_t *xc,
		xhc_rank_info_t *rank_info, int allreduce_count,
	...
	OPAL_LIST_FOREACH_DECL(item, xc->reduce_queue, xhc_rq_item_t) {
		...
		if(!xc->member_info[member].init
			...
			int ret = xhc_allreduce_attach_member(xc, member, rank_info,
		...
		if(xc->member_info[member].init && item->count < allreduce_count) {
			int elements = opal_min(xc->coll_chunk_elements,
				allreduce_count - item->count);
...
static void xhc_allreduce_do_reduce(xhc_comm_t *xc, xhc_rq_item_t *member_item,
	...
	int elements = opal_min(xc->coll_chunk_elements,
		allreduce_count - member_item->count);
	...
	#if LEADER_RD_POLICY == 2
		if(xc->is_coll_leader && xc->reduce_worker_count == xc->size - 1)
			member_item->count = allreduce_count;
		else
			member_item->count += elements * xc->reduce_worker_count;
	#else
		member_item->count += elements * xc->reduce_worker_count;
	#endif
}
...
int mca_coll_xhc_allreduce_internal(const void *sbuf, void *rbuf, int count,
	...
	xhc_allreduce_calc_chunk_sizes(data->comms, data->comm_count, count, dtype_size);
	xhc_allreduce_init_local(data->comms, data->comm_count, count, pvt_seq);
	
	xhc_allreduce_init_comm(data->comms, data->comm_count, rbuf, do_cico, rank, pvt_seq);
	xhc_allreduce_init_member(data->comms, data->comm_count, data->rank_info,
		(void *) sbuf, rbuf, count, do_cico, rank, pvt_seq);
	
	void *local_cico = xhc_get_cico(data->rank_info, data->comms[0].manager_rank);
...
_allreduce: {
	...
	xhc_comm_t *bcast_comm =
		xhc_allreduce_bcast_src_comm(data->comms, data->comm_count);
	...
	for(size_t bytes_done = 0; bytes_done < bytes_total; ) {
		for(int i = 0; i < data->comm_count; i++) {
			...
			xhc_comm_t *xnc = (i < data->comm_count - 1 ? &data->comms[i+1] : NULL);
			
			if(do_cico && i == 0 && xc->my_member_ctrl->reduce_ready < count) {
				xhc_allreduce_cico_publish(xc, (void *) sbuf,
					data->rank_info, rank, count, dtype_size);
			}
			...
			if(xc->is_coll_leader) {
				...
				if(xnc && completed > xnc->my_member_ctrl->reduce_ready) {
					...
					if(bytes_fully_reduced > bytes_done) {
						for(int k = 0; k < data->comm_count; k++) {
							volatile xf_size_t *brp =
								&data->comms[k].comm_ctrl->bytes_ready;
			...
			if(xc->my_member_ctrl->reduce_done < count) {
				...
				int ret = xhc_allreduce_reduce_get_next(xc,
					data->rank_info, count, dtype_size, do_cico,
			...
			if(CHECK_FLAG(&bcast_comm->comm_ctrl->coll_seq, pvt_seq, 0)) {
				...
				if(!bcast_comm->member_info[leader].init) {
					...
					xhc_allreduce_attach_member(bcast_comm, leader,
						data->rank_info, bytes_total, do_cico, pvt_seq);
				}
		...
		if(bcast_comm && bcast_leader_joined) {
			...
			xhc_allreduce_do_bcast(data->comms, data->comm_count,
	...
	xhc_allreduce_do_ack(data->comms, data->comm_count, pvt_seq);
...
_reduce: {
	...
	while(completed_comms < data->comm_count) {
		for(int i = completed_comms; i < data->comm_count; i++) {
			xhc_comm_t *xc = &data->comms[i];
			xhc_comm_t *xnc = (i < data->comm_count - 1 ? &data->comms[i+1] : NULL);
...
_finish:
	...
	if(!do_cico)
		xhc_allreduce_disconnect_peers(data->comms, data->comm_count);
