...
static void xhc_bcast_try_leader(xhc_comm_t *comms, int comm_count,
	ompi_proc_t *root_proc, xf_sig_t seq) {
	...
	for(int i = 0; i < comm_count; i++) {
		...
		if(PROC_IS_LOCAL(root_proc, comms[i].locality))
			break;
...
int mca_coll_xhc_bcast(void *buf, int count, ompi_datatype_t *datatype, int root,
	...
	ompi_proc_t *root_proc = (rank == root ? NULL : data->rank_info[root].proc);
	xhc_bcast_try_leader(data->comms, data->comm_count, root_proc, pvt_seq);
	...
	xhc_bcast_children_init(data->comms, data->comm_count, buf,
	...
	for(int i = 0; i < data->comm_count; i++) {
		data->comms[i].my_member_ctrl->member_seq = pvt_seq;
		if(!data->comms[i].is_coll_leader) break;
	}
	
	xhc_comm_t *src_comm = xhc_bcast_src_comm(data->comms, data->comm_count);
	...
	if(!do_cico)
		...
		src_buffer = xhc_get_cico(data->rank_info, src_ctrl->cico_id);
	...
	while(bytes_done < bytes_total) {
		...
		if(!do_cico) {
			int err = xhc_copy_from(&data->rank_info[src_ctrl->leader_rank],
				data_dst, data_src, copy_size, src_ctrl->access_token);
			if(err != 0) return OMPI_ERROR;
		} else {
			memcpy((data->comms[0].is_coll_leader
		...
		xhc_bcast_children_set_bytes_ready(data->comms,
			data->comm_count, bytes_done);
		
		if(do_cico && data->comms[0].is_coll_leader) {
	...
	xhc_bcast_do_ack(data->comms, data->comm_count, pvt_seq);
