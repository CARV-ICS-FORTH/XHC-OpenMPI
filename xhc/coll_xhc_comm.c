...
static void xhc_comms_divide(xhc_comm_t *xc, xhc_hierarchy_t hierarchy,
		int *rank_list,xhc_peer_info_t *peer_info) {
	
	int max_members = hierarchy.max_members;
	int split = hierarchy.split;
	
	if(!(max_members > 0 || split > 1))
		return;
	...
	if(max_members > 0) {
		...
		owner_id = (xc->my_id / max_members) * max_members;
		
		xc->my_id = xc->my_id - owner_id;
		xc->owner_rank = rank_list[owner_id];
		xc->size = (owner_id + max_members > xc->size ?
			xc->size - owner_id : max_members);
	} else if(split > 1) {
		/* Split up the comm that we created on this level to this
		 * many comms. The last one might contain more members than
		 * the rest. */
		
		int piece_size = xc->size / split;
		int group_id = xc->my_id / piece_size;
		
		owner_id = group_id * piece_size;
		
		xc->my_id = xc->my_id - owner_id;
		xc->owner_rank = rank_list[owner_id];
		xc->size = (group_id == split - 1 ?
			xc->size - owner_id : piece_size);
	}
	...
	for(int m = 0; m < old_size; m++) {
		...
		peer_info[rank_list[m]].locality &= ~xc->locality;
	}
	
	memmove(&rank_list[0], &rank_list[owner_id],
		xc->size * sizeof(rank_list[0]));
}
...
int mca_coll_xhc_comms_make(ompi_communicator_t *ompi_comm,
	...
	rank_list = malloc(n_ranks * sizeof(int));
	
	if(!comms || !ds_list || !candidate_list || !rank_list)
	...
	for(int h = 0; h < config->hierarchy_len; h++) {
		...
		*xc = (xhc_comm_t) {
			.locality = config->hierarchy[h].loc,
		...
		/* Handle dividing virtual hierarchy modifiers (split, max members).
		 * See xhc_component_parse_hierarchy() and xhc_hierarchy_create(). */
		xhc_comms_divide(xc, config->hierarchy[h], rank_list, peer_info);
